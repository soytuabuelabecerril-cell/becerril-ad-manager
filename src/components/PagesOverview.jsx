...
const PagesOverview = () => {
  ...
  if (loading) {
    return <div className="p-4 bg-white rounded-xl shadow-sm border border-gray-100 flex items-center justify-center">Loading overviews...</div>;
  }
    ...
    <div className="space-y-6 mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col items-center justify-center">
          ...
          <span className="text-sm text-gray-500 uppercase tracking-wider font-semibold mt-1">Total Pages</span>
        </div>
        <div className="bg-white p-6 rounded-xl border border-green-100 shadow-sm flex flex-col items-center justify-center">
          ...
          <span className="text-sm text-green-600 uppercase tracking-wider font-semibold mt-1">Available</span>
        </div>
        <div className="bg-white p-6 rounded-xl border border-red-100 shadow-sm flex flex-col items-center justify-center">
          ...
          <span className="text-sm text-red-600 uppercase tracking-wider font-semibold mt-1">Reserved</span>
        </div>
      </div>
            ...
            {pages.map(page => (
              <div key={page.page_number} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                <div className="flex items-center gap-4">
                  <span className="font-bold text-gray-700 w-12">Pg {page.page_number}</span>
                  <span className="text-sm text-gray-600">{page.ad_type || 'No assignment'}</span>
                </div>
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                  isOccupied(page) ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                }`}>
                  {isOccupied(page) ? 'Reserved' : 'Available'}
                </span>
              </div>
            ))}
          </div>
